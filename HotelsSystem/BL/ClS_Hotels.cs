...
public class ClS_Hotels
{
    ...
    {
        return await _db.SaveData<T, dynamic>("Pro_InsertUpdateHotels", new { select = SelectPro, ID = ValID, HotelTypeID = HotelTypeID, HotelName = HotelName, HotelAddress = HotelAddress, StarNumber = StarNumber, NumberOfRooms= NumberOfRooms, DirectorateID = DirectorateID, WorkPointID = WorkPointID, Note = Note, EntryBy = SessionValue });
    }
    ...
    {
        var result = await _db.GetMultiple<HotelTypesComboBox, DirectorateInfo>(
                "Pro_GetCMB",
